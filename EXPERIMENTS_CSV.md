...
`/data/anon/perepoch/runs/*/bank` (50 models each, 11 configs) make regenerating those
...
every train set including 256k. Canonical copies on the Hub: `anonymous/bergson-smollm2-scaling`
...
`/data/anon/perepoch/runs/<name>/bank`; the eps1e-17 banks under
`/data/anon/s16k_{adamw,muon}/merged` (models kept, base-training trajectories deleted —
...
5. **Standing assets:** 11 per-epoch banks (`/data/anon/perepoch/runs/*/bank`, 50
   models each) and the two 100-model banks (`/data/anon/s16k_{adamw,muon}/merged`)
...
- **QK-norm (and native architecture modifications generally).** Cut from the
  ...
  implementation is ready (`gpt2_custom/`, `anonymous/gpt2-custom` on the Hub,

...
DEVS=$1; PORT=$2; ID=$3
C=/data/anon/metasmoothness/configs/tuning/${ID}_s42.yaml
RUN=/data/anon/paper_runs/tuning/${ID}_s42
CLAIM=/data/anon/paper_runs/_claims/${ID}__tune
...
mkdir -p /data/anon/paper_runs/_claims 2>/dev/null
...
setsid nohup env CUDA_VISIBLE_DEVICES="$DEVS" MASTER_PORT="$PORT" PYTHONNOUSERSITE=1 \
  HF_HUB_OFFLINE=1 PYTHONPATH=/data/anon/bergson-main-paper-429 \
  /home/anon/envs/paper/bin/python -s -P -m bergson "$C" \

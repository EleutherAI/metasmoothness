...
set -u
E=/data/anon/paper_runs/experiments/qwen15b_64k_bs256
L=/data/anon/paper_runs/_launch
NODES="anon-ord-0 poppy-0 daisy-0 wisteria-0 jasmine-0 allium-0 lily-0 clover-0 heather-0 orchid-0 bellflower-0 lotus-0 node-x-0 shivam2-0 louis-ord-0 shared-ord-0 violet-0 yarrow-0"
...
for node in $(cat /tmp/q64_nodes.txt); do
  kubectl cp /tmp/q64_launch_$node.sh anon-ord-0:$L/q64_launch_$node.sh >/dev/null 2>&1
done
for node in $(cat /tmp/q64_nodes.txt); do
  timeout 300 kubectl exec $node -- su - anon -c "bash $L/q64_launch_$node.sh" 2>&1 | sed "s/^/[$node] /" &
done; wait

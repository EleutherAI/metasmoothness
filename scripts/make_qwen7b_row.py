...
T = Path("/data/anon/paper_runs/tuning")
DS = "/data/anon/datasets_local"
MODEL = "/data/anon/models/Qwen2.5-7B"
mode = sys.argv[1]
...
if mode == "sweep":
    for lr in (5e-6, 1e-5, 2e-5, 4e-5):
        ...
        run = f"/data/anon/paper_runs/tuning/tune_qwen7b_4k_bs256_lr{lr:g}_s42"
        m["run_path"] = run
        doc["run_path"] = run
    ...
    m["lr_schedule"]["lr"] = lr
    m["lr_schedule"]["lr_end"] = lr / 10
    run = "/data/anon/paper_runs/experiments/qwen7b_4k_bs256"
    m["run_path"] = run
    doc["run_path"] = run

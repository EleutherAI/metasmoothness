...
# ten minutes later and verifies the run is really on a GPU. See PROJECT_RULES.md.
set -u
CFG="$1"; GPUS="$2"; PORT="$3"; NAME="$4"
LOGS=/data/anon/paper_runs/_logs
REG=$LOGS/launch_registry.tsv
# The newer pods (wisteria/jasmine/violet/clover) run as root with no anon home,
# so /home/anon/envs is absent -- but the same pinned env is on the shared volume.
# Pick whichever exists rather than hardcoding one.
if [ -x /home/anon/envs/paper/bin/python ]; then
  PY=/home/anon/envs/paper/bin/python
else
  PY=/data/anon/envs/paper/bin/python
fi
...
if [ $# -ge 5 ]; then
  ...
  BERG=/data/anon/bergson-filter
else
  BERG=/data/anon/bergson-main-paper-429
fi

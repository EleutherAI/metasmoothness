...
set -u
REG=/data/anon/paper_runs/_logs/launch_registry.tsv
LOGS=/data/anon/paper_runs/_logs
MIN_AGE=${1:-600}
NOW=$(date +%s)
ANY_DEAD=0

reg=$(timeout 90 kubectl exec bellflower-0 -- su - anon -c "cat $REG 2>/dev/null" 2>/dev/null)
...
while IFS=$'\t' read -r iso epoch node gpus name pid cfg; do
  ...
  out=$(timeout 90 kubectl exec "$node" -- su - anon -c "

...
sys.path.insert(0, "/data/anon/metasmoothness/scripts")
from axes import is_cut  # noqa: E402

EXP = "/data/anon/metasmoothness/experiments.csv"
DELTAS = "/data/anon/metasmoothness/data/filter_deltas.csv"
...
for scorer, lds_col, dcol in (("MAGIC", "magic_lds", "magic"), ("EK-FAC", "ekfac_lds", "ekfac")):
    ...
    for rid, r in exp.items():
        if is_cut(rid) or not r.get(lds_col):
            continue
        root = next((f"/mnt/ssd-{b}/anon/paper_runs/experiments/{rid}"
                     for b in (2, 1)
                     if os.path.isdir(f"/mnt/ssd-{b}/anon/paper_runs/experiments/{rid}")), None)

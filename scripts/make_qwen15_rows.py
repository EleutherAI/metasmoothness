...
Qwen chain dependency:
    ...
    2. Run `scripts/heldout_eval.py --heldout /data/anon/datasets_local/heldout_4k_qwen_v2.hf .../retrained/base`
...
C = Path("/data/anon/metasmoothness/configs/experiments")
DS = "/data/anon/datasets_local"
MODEL = "/data/anon/models/Qwen2.5-1.5B"
...
if sys.argv[1] == "sweep":
    ...
    for lr in (2e-5, 5e-5, 1e-4, 2e-4):
        ...
        run = f"/data/anon/paper_runs/tuning/tune_qwen15b_{tag}_bs256_lr{lr:g}_s42"
        m["run_path"] = run
        doc["run_path"] = run
    ...
    for n in NS:
        ...
        run = f"/data/anon/paper_runs/experiments/{rid}"
        m["run_path"] = run
        doc["run_path"] = run

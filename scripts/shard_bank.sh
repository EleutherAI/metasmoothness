...
R=""
for b in 1 2; do
  d=/mnt/ssd-$b/anon/paper_runs/experiments/$RID
  [ -d "$d" ] && R=$d && break
done
...
CLAIM=/data/anon/paper_runs/_claims/${RID}__bank${START}_${STOP}
mkdir "$CLAIM" 2>/dev/null || { echo "SKIP: claimed by $(cat "$CLAIM/host" 2>/dev/null)"; exit 1; }
hostname > "$CLAIM/host"

CFG=$R/bank_shard_${START}_${STOP}.yaml
/home/anon/envs/paper/bin/python -s -P - "$SRC" "$CFG" "$START" "$STOP" "$OUT" <<'PY'
import sys, yaml
...
setsid nohup env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  PYTHONPATH=/data/anon/bergson-filter \
  /home/anon/envs/paper/bin/python -s -P -m bergson "$CFG" \

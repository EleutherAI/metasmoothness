...
set -u
NODES="lotus-0 anon-ord-0 secret-ord-0 allium-0 shared-ord-0 bellflower-0 iris-0 wisteria-0 jasmine-0 violet-0 clover-0 yarrow-0 poppy-0 heather-0 orchid-0"
INTERVAL=${1:-20}
for node in $NODES; do
  timeout 90 kubectl exec "$node" -- su - anon -c '
    h=$(hostname)
    P=$(pgrep -u anon -f "bin/python -s -P" | tr "\n" ",")
    P=${P%,}
    [ -z "$P" ] && { printf "  %-13s idle\n" "$h"; exit 0; }
    ...
    ranks=$(pgrep -u anon -cf "multiprocessing.spawn")

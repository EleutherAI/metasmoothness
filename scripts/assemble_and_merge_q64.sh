#!/bin/bash
# Post-run step for qwen 64k plan A. Run on anon-ord as ROOT via: kubectl exec anon-ord-0 -- bash /tmp/assemble_and_merge_q64.sh <1pct|top40>
...
E=/data/anon/paper_runs/experiments/qwen15b_64k_bs256; PY=/data/anon/envs/paper/bin/python; M=/data/anon/metasmoothness/scripts; D=/data/anon/datasets_local
echo "== 1. bank shards"
for i in 0 1 2; do
...
cp -f $E/bank_${tag}_s0/subsets.json $B/subsets.json; chown -R anon:anon $B
...
for a in $(seq 0 19); do
  ...
  su - anon -c "cd /tmp && CUDA_VISIBLE_DEVICES=0 $PY $M/bank_merge.py --run $R --bank $B --query $D/query_20_qwen_q${a}_$((a+1)).hf" 2>&1 | grep -E "^wrote|Error" | sed "s#$E/##" && merged=$((merged+1))
done
echo "shards merged=$merged pending=$pending"
if [ $merged -eq 20 ]; then
  echo "== 3. merge 20 shards"
  su - anon -c "cd /tmp && $PY -P $M/merge_filter_shards.py qwen15b_64k_bs256 --source ekfac --prefix $prefix --force" 2>&1 | tail -2
fi

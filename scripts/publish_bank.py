...
Anon's rule: a bank leaves the node only once everything derived from it exists
...
def find_root(run_id):
    for base in ("/mnt/ssd-2", "/mnt/ssd-1"):
        c = Path(base) / "anon/paper_runs/experiments" / run_id
        if c.is_dir():
            return c
...
def main():
    ...
    # distinction: `anon` is uid 1001 on three nodes and 1000 on the other seven,
    ...
    probe = subprocess.run(
        ["/home/anon/envs/paper/bin/python", "-s", "-P",
         "/data/anon/metasmoothness/scripts/magic_lds.py", str(root)],

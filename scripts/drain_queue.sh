...
#   * Node allowlist per D21: node-x-0 and shivam2-0 are permanently off, and
...
BERG="${2:-}"
NODES="anon-ord-0 secret-ord-0 allium-0 shared-ord-0 bellflower-0 lotus-0 wisteria-0 jasmine-0 violet-0 clover-0 yarrow-0 poppy-0 heather-0 orchid-0"
...
for round in $(seq 1 400); do
  ...
  for idx in "${!JOBS[@]}"; do
    ...
    for node in $NODES; do
      pair=$(timeout 40 kubectl exec "$node" -- su - anon -c \
      ...
      [ -z "$pair" ] && continue
      out=$(timeout 120 kubectl exec "$node" -- su - anon -c \

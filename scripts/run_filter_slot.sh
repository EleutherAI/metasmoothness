...
# `anon` is uid 1001 on iris-0/secret-ord-0/maria-1 and uid 1000 on the other
...
BERGSON=/data/anon/bergson-filter
ENVPY=/home/anon/envs/paper/bin/python
...
LOG=/data/anon/paper_runs/experiments/filter_slot_$(hostname)_${DEVS//,/-}.log
...
" "$RID" 2>/dev/null)
  ...
  RDIR=$(ls -d /mnt/ssd-*/anon/paper_runs/experiments/"$RID" 2>/dev/null | head -1)
  NOBANK=""
  ...
  R=$(ls -d /mnt/ssd-*/anon/paper_runs/experiments/"$RID" 2>/dev/null | head -1)
  ...
  CLAIMS=/data/anon/paper_runs/_claims
  mkdir -p "$CLAIMS"

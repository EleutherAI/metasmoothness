...
for b in ("/mnt/ssd-2", "/mnt/ssd-1"):
    p = Path(b) / "anon/paper_runs/experiments" / args.run_id
    if p.is_dir():
        root = p
        break
if root is None:
...
out_root = Path(args.out_root) / "anon/paper_runs/experiments" / args.run_id \
...
TEMPLATE = Path("/data/anon/paper_runs/experiments/"
...
# /data/anon/bergson-damping/runs/ekfac_vs_n/datasets/, and ssd-1 is full:
...
print("    /home/anon/envs/paper/bin/python -s -P -m bergson %s \\" % out)

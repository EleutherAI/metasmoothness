...
`/data/anon/perepoch/` (`gen_configs.py`, `run_all.sh`, `build_table.py`) on bergson HEAD
...
Checked every MAGIC score tensor on disk under `/data/anon/muon4k`. 560,000 score elements
...
Run dir `/data/anon/muon4k/magic_eps0_bs64_q0`.
...
- The full-run OLMo2 bank's per-query Δloss sd is 0.066 (vs ~0.001 for GPT-2 fine-tuning banks).
  Re-scoring the surviving index (`/data/anon/scratch_olmo/N16k_scores`)
  with different damping/SOURCE/Trackstar does not change it.
- **Code / configs (branch `feat/ms-pretrain` @ `/data/anon/bergson-ms-pretrain`):** window
...
`/data/anon/wikitext_ms_eps1e8_bs64/`) makes this row a measured high-ms/low-MAGIC point:
...
- SmolLM2 eps_root=1e-6 4k adam bank: HF `anonymous/bergson-smollm2-lds-4k` (+ `run_config.yaml`, `subsets.json`). Size-scaling banks: `runs/ekfac_vs_n/N{4,8,16,32}k`. adam eps_root=0 bank: `/data/anon-adam-shampoo/epsroot0_4k_bank/` (code `b3790ba9`). muon banks: `/data/anon/muon4k/{run,run_1e-4,run_eps0_5e-5,run_eps0_1e-4}/N4k` (differ only in eps_root and lr).
- SmolLM2 scoring summary.csv under `/data/anon-adam-shampoo/*/validate/` and `/data/anon/muon4k/**/validate/`; scoring code `1ba43f92` worktree (+ `feat/shampoo-quarter-power` for the Shampoo power variants). WikiText run dirs under `runs/`.
- All banks above: `data.chunk_length = 0`.
- muon N × eps_root metasmoothness sweep (2026-07-27): `/data/anon/muon_ms_steps/` — driver
  ...
  Run from `/data/anon/bergson-damping`; **requires `PYTHONPATH=<repo>`** — the `bergson`
...
- MAGIC finiteness audit: score tensors read from `/data/anon/muon4k/{magicroll_*,magic_*}/q*/scores.pt`.
...
**Aggregate-query metrics are non-standard and must not be used** (see the PROJECT_RULES.md rule: LDS is
...
`10874f93` against the 100-model bank at `/data/anon/s16k_adamw/merged`
(scores: `/data/anon/s16k_adamw/ekfac_scores`; estimator:
...
muon twin (same pipeline, bank `/data/anon/s16k_muon/merged`, scores
`/data/anon/s16k_muon/ekfac_scores`):
...
at `/data/anon/s16k_adamw/ms`). The adamw anchor is now a complete
...
at `/data/anon/s16k_muon/ms`). Both anchor rows are complete three-metric
configs:
...
the fresh retrain (`/data/anon/paper_runs/d9_magic_base/retrained/base`; scores at
`/data/anon/paper_runs/envdrift_ekfac/scores`):
...
| muon 16k bs16 | 5e-5 | 0.8850 | [0.8708, 0.8967] | 100 | 20 | lotus-0 A100, nproc 2; retrains 92-99 sliced | 79c08dce |

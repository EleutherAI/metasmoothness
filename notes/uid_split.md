# `anon` is two different uids on this fleet, and bergson writes mode 0600

    uid 1001   iris-0, secret-ord-0, maria-1
    uid 1000   bellflower-0, allium-0, shared-ord-0, node-x-0, lotus-0,
               shivam2-0, anon-ord-0
...
EK-FAC scoring for `plan_adam_eps1e17_32k_bs32` died on anon-ord-0 with

    FileNotFoundError: .../base/model/model.safetensors

while `ls -la` on that same node showed the file, 497 MB, right there. Reading
it worked from iris-0 and failed from bellflower-0, allium-0 and anon-ord-0 --

...
Anon asked for a gpt2-medium configuration at ms >= 0.98 with the same step count
...
anchor -- Anon's constraint, with the extra data compensating for the doubled
...
the "same number of training steps" constraint Anon set.
...
Anon set, with data scaled to match as she allowed.

...
(with checkpoints) at /data/anon/paper_runs/d9_magic_base, but per D15 it does NOT
...
The custom GPT-2 implementation gets finalized and uploaded to the Anonymous Hugging Face
...
the bergson library per Anon) and uploaded as
`anonymous/gpt2-custom` (private, remote code). The implementation makes the equivalence
...
plumbing coerces types, so a row runs with `model: anonymous/gpt2-custom` and
...
Anon may waive the literal run on that argument or keep it as a pipeline test. Then
...
still goes to Anon for sign-off **before** any model-size run starts, including the tuning
sweeps; the tuning groups stay registered but are not claimable until sign-off.

**Cost plan signed off (Anon, 2026-08-23).** gpt2-medium is claimable. Plan as costed,
on three borrowed A100-80GB pods (node-x-0, maria-1, shivam2-0) available ~24 h:
...
The open question this raises for Anon is whether a 355M scaling point is worth
...
**Escalation rule (revised 2026-08-22 by Anon):** 20 queries (`query_20.hf`) unless a
...
| anchor split-half | LDS from subsets 0-49 vs 50-99 | adamw 0.9294 vs 0.9336 (0.004); muon 0.8451 vs 0.8429 (0.002) | /data/anon/s16k_{opt}/eval_q20/validation.csv |
...
### D17. GPU type is part of run identity — CAUSE CONFIRMED; scope still needs Anon
...
**CONFOUND IN THIS MEASUREMENT (found 2026-08-24, after Anon asked whether the
...
3.11.15, via `/data/anon/envs/paper` (same package versions, different
...
|---|---|---|---|
| `plan_adam_eps1e17_16k_bs128` | node-x-0 (A100) | pairs against muon bs128, measured on A40 | optimizer contrast vs GPU type |
...
**SCOPE RULING (Anon, 2026-08-24): accept and label. No re-runs.** The
...
exist at `/data/anon/paper_runs/d9_magic_base`); or (b) accept a mixed-provenance
...
found only because Anon asked for node-x-0 to be cleared for unrelated reasons.
...
## D21: node allowlist (Anon, 2026-08-27) -- HARD LIMIT

Anon has been pinged for our GPU usage. We are allowed ONLY these nodes:
...
Permanently OFF, do not launch, do not "just check": **node-x-0**, **shivam2-0**,
**maria-1**.
...
## D22: no more LDS, no more retrain banks (Anon, 2026-08-27)
...
Three separate wedges, all the same shape, all on ssd-1:
    ...
    node-x-0            bank_build            D state, 23h, node effectively lost
    iris-0              ekfac 4000-step       D state, 9h16m, ~9h of work lost
    anon-ord-0         ekfac 4000-step       D state, 9h39m, ~9h of work lost
...
Rules:
  ...
  1. **PYTHONPATH off ssd-1.** Use `/data/anon/bergson-filter` and
     `/data/anon/bergson-main-paper-429`. Both mirrors exist and are
  ...
  3. **Datasets on ssd-2.** Already handled by gen_bank.py/gen_filter.py, which
     rewrite dataset paths to /data/anon/datasets_local.
